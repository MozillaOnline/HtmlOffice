...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  exclude-result-prefixes="a pic w r wp v w10 o rels oox">
  ...
  *************************************************************************
  ...
  <xsl:template name="InsertFrameSizeAttributes">
    ...
    <xsl:if test="$framePr/@w:w">
      <xsl:attribute name="svg:width">
        <xsl:value-of select="concat($framePr/@w:w * 2.54 div 1440.0, 'cm')" />
      </xsl:attribute>
    </xsl:if>

    <xsl:if test="$framePr/@w:h">
      <xsl:attribute name="svg:height">
        <xsl:value-of select="concat($framePr/@w:h * 2.54 div 1440.0, 'cm')" />
      </xsl:attribute>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="InsertFrameMarginForStyle">
    ...
    <!-- vertical margin -->
    <xsl:variable name="vertMargin" select="concat($framePr/@w:vSpace * 2.54 div 1440.0, 'cm')" />

    <!-- horizontal margin -->
    <xsl:variable name="horzMargin" select="concat($framePr/@w:hSpace * 2.54 div 1440.0, 'cm')" />

    <xsl:attribute name="fo:margin-bottom">
      <xsl:value-of select="concat($framePr/@w:vSpace * 2.54 div 1440.0, 'cm')"/>
    </xsl:attribute>
    ...
    <xsl:attribute name="fo:margin-left">
      <xsl:value-of select="concat($framePr/@w:hSpace * 2.54 div 1440.0, 'cm')"/>
    </xsl:attribute>
      ...
      <xsl:attribute name="text:style-name">
        <xsl:choose>
          <xsl:when test="$paragraph/w:pPr/w:pStyle/@w:val">
            <xsl:value-of select="concat('ooc-NCNameFromString', '-oop-', $paragraph/w:pPr/w:pStyle/@w:val, '-ooe')"/>
          </xsl:when>
          <xsl:otherwise>
