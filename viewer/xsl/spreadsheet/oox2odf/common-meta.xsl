  ...
  <xsl:template name="meta">
    ...
    <office:document-meta office:version="1.1">
      <office:meta>
        ...
        <!-- title -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/dc:title" mode="meta"/>
        <!-- description -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/dc:description"
            mode="meta"/>
        <!-- creator -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/dc:creator" mode="meta"/>
        <!-- creation date -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/dcterms:created"
            mode="meta"/>
        <!-- last modifier -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/cp:lastModifiedBy"
            mode="meta"/>
        <!-- last modification date -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/dcterms:modified"
            mode="meta"/>
        <!-- last print date -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/cp:lastPrinted"
            mode="meta"/>
        <!-- subject -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/dc:subject" mode="meta"/>
        <!-- editing time -->
        <xsl:apply-templates select="key('Part', 'docProps/core.xml')/Properties/TotalTime"
            mode="meta"/>
        <!-- revision number -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/cp:revision" mode="meta"/>
        <!-- custom properties-->
        <xsl:apply-templates
            select="key('Part', 'docProps/custom.xml')/cust-p:Properties/cust-p:property"
            mode="meta"/>
        <!-- keywords -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/cp:keywords" mode="meta"/>
        <!-- language -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/dc:language" mode="meta"/>
        <!-- category -->
        <xsl:apply-templates
            select="key('Part', 'docProps/core.xml')/cp:coreProperties/cp:category" mode="meta"/>
        <!-- content status -->
        <xsl:apply-templates
                    select="key('Part', 'docProps/core.xml')/cp:coreProperties/cp:contentStatus" mode="meta"/>
        <!-- document statistics -->
        <xsl:call-template name="GetDocumentStatistics"/>
        <!-- Total editing time and other app properties -->
        <xsl:apply-templates
					select="key('Part', 'docProps/app.xml')/ex:Properties/*" mode="meta"/>
      </office:meta>
